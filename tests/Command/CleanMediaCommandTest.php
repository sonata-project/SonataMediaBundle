...
class CleanMediaCommandTest extends FilesystemTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->fileSystemLocal->expects(static::once())->method('getDirectory')->willReturn($this->workspace);
    ...
    public function testExecuteDirectoryNotExists(): void
    {
        ...
        $this->pool->expects(static::once())->method('getContexts')->willReturn(['foo' => $context]);
    ...
    public function testExecuteFilesExists(): void
    {
        ...
        $this->mediaManager->expects(static::once())->method('findOneBy')
            ->with(static::equalTo(['id' => 1, 'context' => 'foo']))
            ->willReturn([$media]);
        $this->mediaManager->expects(static::once())->method('findBy')
            ->with(static::equalTo(['providerReference' => 'qwertz.ext', 'providerName' => ['fooprovider']]))
            ->willReturn([$media]);
